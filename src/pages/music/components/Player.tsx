...
export default function Player({ album, title, position, artist, imageUrl, isPlaying, setIsPlaying }: {
  album: string
  title: string
  position: number
  artist: string
  ...
  // const { album, title, artist, imageUrl, playing } = props
  ...
  useEffect(() => {
    ...
    audioPlayer.current!.src = `/mp3/album${album}/song${position}.mp3`
    audioPlayer.current!.currentTime = 0
    if (isPlaying){audioPlayer.current!.play()}
    if (isPlaying===false){audioPlayer.current!.pause()}
